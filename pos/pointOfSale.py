...
class SalesWindow(QWidget):
    ...
    def submit_sale(self):
        ...
        # Prepare items with product name for receipt
        receipt_items = []
        for product_id, qty, price in items:
            product_name = self.db.get_product_name_by_id(product_id)  # you'll need to implement this if not existing
            receipt_items.append((product_name, qty, price))

        self.receipt_window = ReceiptWindow(phone, receipt_items, total_amount)
