...
""")
    ...
    def update_product_quantity(self, product_id, quantity_sold, conn=None):
        """Reduce stock quantity when a product is sold"""
        if conn is None:
            conn = self.get_connection()
            close_conn = True
        else:
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE product SET reorder_level = reorder_level - ? WHERE product_id = ?",
    ...
    def save_sale(self, customer_id, phone, items, total):
        ...
        for product_id, qty, price in items:
            ...
            self.update_product_quantity(product_id, qty, conn)  # reuse same connection
    ...
    def get_product_name_by_id(self, product_id):
        conn = self.get_connection()
        c = conn.cursor()
        c.execute("SELECT name FROM product WHERE product_id = ?", (product_id,))
        result = c.fetchone()
        conn.close()
        return result[0] if result else None


    def get_sales_history(self, phone=None):
        conn = self.get_connection()
        c = conn.cursor()
        if phone:
            c.execute("""
                SELECT s.sale_id, s.customer_id, s.phone, s.total_amount, s.sale_date,
